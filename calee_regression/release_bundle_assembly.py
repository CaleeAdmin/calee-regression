...
def write_release_bundle(assembly: BundleAssembly, out_dir: "Path | str") -> Path:
    ...
    partial/invalid bundle. The destination directory is created if absent;
    existing unrelated files are left alone (an ``--out`` reused across
    releases still ends up self-consistent because every APK/manifest/
    checksum filename this writes is deterministic from the inputs)."""
    ...
    (out / MANIFEST_NAME).write_text(json.dumps(assembly.manifest, indent=2) + "\n", encoding="utf-8")
    return out
