...
| Symptom | Cause | Fix |
|---|---|---|
| A scenario reports `blocked` with "Could not start an Appium session" or "Appium endpoint … is unavailable" | Appium is down, or the device disconnected | Every tablet command (`run`, `suite`, `run-repeat`, `prepare`, and the `focused-verify` orchestration) now **ensures Appium itself** before creating a session (Workstream 1) — you no longer need to run `prepare` first in the same shell. Confirm the device is connected and that `appium` is installed/on PATH, then retry |
...
| `doctor` reports `appium_reachable: error` | `doctor` only *checks* — it never starts Appium | Every tablet command (`run`/`suite`/`run-repeat`/`prepare`/`focused-verify`) now starts Appium for you if it isn't already running (Workstream 1), so just run the command you actually want; only `doctor` and `stop-appium` never start it. To start it manually: `appium --base-path /wd/hub --allow-insecure uiautomator2:adb_shell` |
