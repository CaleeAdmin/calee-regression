...
| `mobile-android` | `CaleeMobile-Regression` | `flutter test integration_test -d android` in `ui/`, or `03 Test CaleeMobile Android.command` | Standard | Signed-in CaleeMobile session on an Android device/emulator | Recommended (REG-* fixture) once UI flows start asserting on it | Yes (or an Android emulator) | Not yet — the UI suite hasn't been executed even once (no toolchain available when this suite was last touched); do not treat a claimed pass as verified until it has actually run |
| `mobile-ios` | `CaleeMobile-Regression` | `flutter test integration_test -d ios` in `ui/`, or `04 Test CaleeMobile iPhone.command` | Standard | Signed-in CaleeMobile session on an iPhone/simulator | Same as `mobile-android` | Yes (a Mac with Xcode; simulator counts) | Not yet, for the same reason as `mobile-android` |
...
| `full-release` (alias of `full-tester`) / full solution | Both (orchestrated) | `05 Test Full Calee Solution.command` | Extended | Prepared tablet; CaleeMobile if attached | Yes | No for tablet+API; yes for the mobile UI suites | Yes — see `docs/RELEASE_POLICY.md` |
