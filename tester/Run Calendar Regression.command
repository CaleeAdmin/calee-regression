...
echo "Running calendar regression — only run this on a PREPARED tablet/emulator with a logged-in demo account."
...
if [ $STATUS -eq 0 ]; then
    ...
    echo ""
    echo "FAILED: calendar — open the report (Open Latest Report.command) for details."
fi
